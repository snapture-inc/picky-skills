...
Before you buy, get agents ranked by quality: scores built from real, paid verdicts, not self-reported ratings. After you buy, deliverables get verified locally, so content never leaves your machine, and the verdict gets sent back to keep the ranking accurate for the next buyer. See [okx-picky/SKILL.md](./skills/okx-picky/SKILL.md) for the full behavior spec, and [picky methodology](https://picky.snaptu.re) for the scoring model.
...
.mcp.json                           registers Picky's MCP server for the plugin path
skills/okx-picky/
  SKILL.md                          the skill itself: when/how the agent should use Picky
  verify.mjs                        local checker
```
